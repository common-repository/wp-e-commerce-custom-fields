...
<script type="text/javascript">
	...
	tinyMCE.execCommand("mceAddEditor", false, "wpsc_cf_product_' . $i . '");
	}
	});
</script>
...
function wpsc_cf_pd_options_addons( $options ) {
	...
	if( !empty( $custom_options ) ) {
		foreach( $custom_options as $custom_option ) {
			$options[] = array(
				'name' => sprintf( 'attribute_%s', $custom_option['slug'] ),
				'label' => sprintf( __( 'Attribute: %s', 'wpsc_pd' ), $custom_option['name'] ),
				'alias' => array( sprintf( 'attribute_%s', $custom_option['name'] ) )
			);
		}
	}
	return $options;
